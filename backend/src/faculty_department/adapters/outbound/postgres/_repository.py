...
class PostgresRepository[T](ABC):
    ...
    def __init__(self, engine: AsyncEngine, *, label: str, table: Table, key: str) -> None:
        self._engine = engine
        self._label = label
        self._table = table
        self._key = key
        self._identities: dict[str, T] = {}

    # ---- what each aggregate supplies ----

    @abstractmethod
    def identity_of(self, aggregate: T) -> str:
        """The value of this aggregate's primary key."""

    @abstractmethod
    def row_of(self, aggregate: T) -> dict[str, Any]:
        """The parent row, primary key included."""
    ...
    def child_tables(self) -> Sequence[tuple[Table, str]]:
        """Child tables to read, each with the column naming its parent."""
        return ()
    ...
    async def _add(self, aggregate: T) -> None:
        ...
        async with translating(f"{self._label} {key}"), self._engine.begin() as conn:
    ...
    async def _save(self, aggregate: T) -> None:
        key = self.identity_of(aggregate)
        row = dict(self.row_of(aggregate))
        row.pop(self._key)
        async with translating(f"{self._label} {key}"), self._engine.begin() as conn:
            result = await conn.execute(
                update(self._table).where(self._table.c[self._key] == key).values(**row)
            )
    ...
    @resilient()
    async def _get(self, key: str) -> T | None:
        async with translating(f"{self._label} {key}"), self._engine.connect() as conn:
            row = (
                await conn.execute(select(self._table).where(self._table.c[self._key] == key))
            ).one_or_none()
            if row is None:
                return None
            return await self._materialise(conn, row, key)
    ...
    async def _find_one(self, whereclause: Any) -> T | None:
        ...
        async with translating(self._label), self._engine.connect() as conn:
            ...
            if row is None:
                return None
            return await self._materialise(conn, row, getattr(row, self._key))
    ...
    async def _list(self, whereclause: Any = None) -> tuple[T, ...]:
        ...
        async with translating(self._label), self._engine.connect() as conn:
            ...
            return tuple(
                [await self._materialise(conn, row, getattr(row, self._key)) for row in rows]
            )

    # ---- internals ----

    async def _materialise(self, conn: AsyncConnection, row: Row[Any], key: str) -> T:
        ...
        """
        children = {
            table: (await conn.execute(select(table).where(table.c[parent_column] == key))).all()
            for table, parent_column in self.child_tables
        }
    ...
    async def _replace_children(self, conn: AsyncConnection, aggregate: T, key: str) -> None:
        ...
        """
        for table, parent_column in self.child_tables:
            await conn.execute(delete(table).where(table.c[parent_column] == key))
        await self._write_children(conn, aggregate)
