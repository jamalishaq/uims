"""The dict behind every in-memory repository in this package.

Five aggregates need the same four operations over a mapping from identifier to object.
Writing that five times would mean five chances to get ``add`` subtly different from one
repository to the next, so it is written once and each repository supplies only the
accessor that pulls an aggregate's identifier out.
...
from faculty_department.ports.errors import AggregateNotFoundError, DuplicateAggregateError
