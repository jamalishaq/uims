...
One aggregate needs the same four things: insert a parent row and its children, update them,
read them back, and list them in the order they were added. Written once here for the reason
``_store.py`` gives for the in-memory equivalent. It is carried here in full rather than
trimmed to what one aggregate uses, because the seven other contexts each hold the same file
and a divergent eighth copy is harder to read than an identical one.
...
is not safe to share, and nothing here is safe to share between concurrent logins.
...
what the in-memory store gave for free by holding live references. Reconstitution
...
from identity.adapters.outbound.postgres._errors import translating
from identity.ports.errors import AggregateNotFoundError
...
class PostgresRepository[T](ABC):
    ...
    def _remember(self, key: Key, aggregate: T) -> T:
        ...
        Unused by this context today, and kept so the file stays the one the others hold.
        """
