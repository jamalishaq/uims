...
class MakeOfferToApplicant:
    ...
    **No transaction spans two aggregates** (CLAUDE.md section 4). A place is claimed on a
    ...
    def execute(self, command: MakeOfferToApplicantCommand) -> OfferDecision:
        ...
        cycle = self._cycles.get(applied_program_id, applicant.session_id)
        if cycle is None:
        ...
        for alternative in self._alternatives_for(applicant):
            considered.append(alternative)
            alternative_cycle = self._offerable(applicant, alternative)
            if alternative_cycle is not None:
                self._claim(applicant, alternative_cycle)
                return OfferMade(
                    applicant_id=applicant.applicant_id,
    ...
    def _alternatives_for(self, applicant: Applicant) -> tuple[str, ...]:
        ...
        """
        policy = self._policies.get(applicant.applied_program_id, applicant.session_id)
        return () if policy is None else policy.alternatives

    def _offerable(self, applicant: Applicant, program_id: str) -> AdmissionCycle | None:
        ...
        *next* alternative had room.
        ...
        way to give it back.
        """
        requirement = self._requirements.get(program_id, applicant.session_id)
        if requirement is None:
            return None
        if not self._rule.qualifies(requirement, applicant.utme_result):
            return None

        cycle = self._cycles.get(program_id, applicant.session_id)
        if cycle is None:
            return None
    ...
    def _claim(self, applicant: Applicant, cycle: AdmissionCycle) -> None:
