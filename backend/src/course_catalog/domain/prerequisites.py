...
class PrerequisiteGraph:
    ...
    def __init__(self, resolve: Callable[[str], Course | None]) -> None:
        self._resolve = resolve

    def chain_for(self, course_id: str) -> tuple[str, ...]:
        ...
        seen = {course_id}
        chain: list[str] = []
        queue = deque(self._direct_prerequisites(course_id))
        ...
        while queue:
            ...
            queue.extend(self._direct_prerequisites(next_id))

        return tuple(chain)

    def ensure_can_require(self, course: Course, prerequisite_id: str) -> None:
        ...
        """
        if course.course_id in self.chain_for(prerequisite_id):
    ...
    def _direct_prerequisites(self, course_id: str) -> tuple[str, ...]:
        course = self._resolve(course_id)
