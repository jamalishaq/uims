...
async def offers_made_on(cycles: AdmissionCycleRepositoryPort, program_id: str) -> int:
    cycle = await cycles.get(program_id, SESSION_ID)
    assert cycle is not None, f"no cycle stored for {program_id}"
    return cycle.offers_made


async def stored(applicants: ApplicantRepositoryPort) -> Applicant:
...
@pytest.fixture
async def screened_application(applicants: ApplicantRepositoryPort) -> None:
    await applicants.add(a_screened_applicant())


@pytest.fixture
async def overflow_chain(
    ...
    """
    await requirements.add(a_chemistry_requirement(PHYSICS))
    await requirements.add(a_science_requirement(MATHEMATICS))
    await policies.add(a_policy((PHYSICS, MATHEMATICS)))


@pytest.mark.usefixtures("screened_application")
class TestAnOfferOnTheProgramAppliedFor:
    async def test_the_applicant_is_offered_the_program_they_asked_for(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        await cycles.add(a_cycle(COMPUTER_SCIENCE))

        decision = await make_offer_to_applicant.execute(COMMAND)
    ...
    async def test_a_direct_offer_is_not_an_alternative_one(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        await cycles.add(a_cycle(COMPUTER_SCIENCE))
    ...
    async def test_the_applicant_now_holds_an_offer_on_that_program(
        self,
        ...
        await make_offer_to_applicant.execute(COMMAND)

        applicant = await stored(applicants)
        assert applicant.status is ApplicationStatus.OFFERED
        assert applicant.offered_program_id == COMPUTER_SCIENCE

    async def test_the_claimed_place_is_persisted_on_the_cycle(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
    ) -> None:
        """The count has to survive the use case, or the next applicant gets the same place."""
        await cycles.add(a_cycle(COMPUTER_SCIENCE, quota=5))

        await make_offer_to_applicant.execute(COMMAND)

        assert await offers_made_on(cycles, COMPUTER_SCIENCE) == 1

    async def test_the_alternatives_are_never_consulted_when_the_program_has_room(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        requirements: ProgramEntryRequirementRepositoryPort,
        ...
        await cycles.add(a_cycle(COMPUTER_SCIENCE))
        await cycles.add(a_cycle(MATHEMATICS))
        await requirements.add(a_science_requirement(MATHEMATICS))
        await policies.add(a_policy((MATHEMATICS,)))

        await make_offer_to_applicant.execute(COMMAND)

        assert await offers_made_on(cycles, MATHEMATICS) == 0

    async def test_the_last_place_can_be_claimed(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        """Quota boundary from the outside: offers_made == quota - 1 still has one to give."""
        await cycles.add(a_cycle(COMPUTER_SCIENCE, quota=3, offers_made=2))

        decision = await make_offer_to_applicant.execute(COMMAND)

        assert isinstance(decision, OfferMade)
        assert await offers_made_on(cycles, COMPUTER_SCIENCE) == 3
...
class TestAnOfferOnAnAlternativeProgram:
    ...
    async def test_the_second_alternative_gets_the_applicant(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        await cycles.add(a_full_cycle(COMPUTER_SCIENCE))
        await cycles.add(a_cycle(PHYSICS))
        await cycles.add(a_cycle(MATHEMATICS))
    ...
    async def test_the_offered_program_differs_from_the_applied_one(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        applicants: ApplicantRepositoryPort,
        cycles: AdmissionCycleRepositoryPort,
    ...
    async def test_no_place_is_claimed_on_the_alternative_they_did_not_qualify_for(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        ...
        assert await offers_made_on(cycles, PHYSICS) == 0
        assert await offers_made_on(cycles, MATHEMATICS) == 1

    async def test_the_full_program_is_left_exactly_as_it_was(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        """QuotaExhausted claims nothing: a full cycle does not become fuller."""
        await cycles.add(a_cycle(COMPUTER_SCIENCE, quota=2, offers_made=2))
        await cycles.add(a_cycle(PHYSICS))
        await cycles.add(a_cycle(MATHEMATICS))

        await make_offer_to_applicant.execute(COMMAND)

        assert await offers_made_on(cycles, COMPUTER_SCIENCE) == 2

    async def test_a_full_first_alternative_is_stepped_past_too(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        requirements: ProgramEntryRequirementRepositoryPort,
        ...
        await cycles.add(a_full_cycle(COMPUTER_SCIENCE))
        await cycles.add(a_full_cycle(MATHEMATICS))
        await cycles.add(a_cycle(STATISTICS))
        await requirements.add(a_science_requirement(STATISTICS))
        await policies.save(a_policy((MATHEMATICS, STATISTICS)))
    ...
    async def test_an_alternative_with_no_published_requirement_is_skipped(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        policies: AlternativeProgramPolicyRepositoryPort,
        ...
        await cycles.add(a_cycle(STATISTICS))
        await cycles.add(a_cycle(MATHEMATICS))
        await policies.save(a_policy((STATISTICS, MATHEMATICS)))

        decision = await make_offer_to_applicant.execute(COMMAND)

        assert isinstance(decision, OfferMade)
        assert decision.program_id == MATHEMATICS
        assert await offers_made_on(cycles, STATISTICS) == 0

    async def test_an_alternative_with_no_open_cycle_is_skipped(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        requirements: ProgramEntryRequirementRepositoryPort,
...
class TestWhenNothingHasAPlaceLeft:
    async def test_the_answer_is_returned_rather_than_raised(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        requirements: ProgramEntryRequirementRepositoryPort,
        ...
        await cycles.add(a_full_cycle(COMPUTER_SCIENCE))
        await cycles.add(a_full_cycle(MATHEMATICS))
        await cycles.add(a_full_cycle(STATISTICS))
        await requirements.add(a_science_requirement(MATHEMATICS))
        await requirements.add(a_science_requirement(STATISTICS))
        await policies.add(a_policy((MATHEMATICS, STATISTICS)))
    ...
    async def test_the_application_is_closed_and_final(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        applicants: ApplicantRepositoryPort,
        cycles: AdmissionCycleRepositoryPort,
        ...
        await cycles.add(a_full_cycle(COMPUTER_SCIENCE))
        await cycles.add(a_full_cycle(PHYSICS))
        await cycles.add(a_full_cycle(MATHEMATICS))
    ...
    @pytest.mark.usefixtures("overflow_chain")
    async def test_not_one_place_was_claimed_anywhere(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        await cycles.add(a_cycle(COMPUTER_SCIENCE, quota=2, offers_made=2))
        await cycles.add(a_cycle(PHYSICS, quota=3, offers_made=3))
        await cycles.add(a_cycle(MATHEMATICS, quota=4, offers_made=4))

        await make_offer_to_applicant.execute(COMMAND)

        assert await offers_made_on(cycles, COMPUTER_SCIENCE) == 2
        assert await offers_made_on(cycles, PHYSICS) == 3
        assert await offers_made_on(cycles, MATHEMATICS) == 4

    async def test_a_program_with_no_published_policy_has_no_fallback(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
    ...
    async def test_an_empty_published_chain_is_the_same_answer(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        policies: AlternativeProgramPolicyRepositoryPort,
    ...
    async def test_a_program_admitting_nobody_is_full_from_the_moment_it_opens(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        """A quota of zero needs no second notion of "closed" anywhere in this flow."""
        await cycles.add(a_cycle(COMPUTER_SCIENCE, quota=0))
    ...
    @pytest.mark.usefixtures("overflow_chain")
    async def test_every_program_examined_is_named_in_order(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
...
class TestWhatTheFlowRefusesToDo:
    async def test_an_unknown_applicant_is_an_error(
        ...
        with pytest.raises(ApplicantNotFoundError):
            await make_offer_to_applicant.execute(MakeOfferToApplicantCommand("app-nobody"))

    @pytest.mark.usefixtures("screened_application")
    async def test_an_applied_program_with_no_open_cycle_is_an_error(
    ...
    async def test_an_unscreened_applicant_is_the_domains_refusal(
    ...
    async def test_an_unscreened_applicant_costs_nobody_a_place(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        applicants: ApplicantRepositoryPort,
        cycles: AdmissionCycleRepositoryPort,
        ...
        assert await offers_made_on(cycles, COMPUTER_SCIENCE) == 0

    @pytest.mark.usefixtures("screened_application")
    async def test_an_applicant_who_already_holds_an_offer_cannot_be_offered_again(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
    ) -> None:
    ...
    @pytest.mark.usefixtures("screened_application")
    async def test_an_application_that_already_ended_cannot_be_offered_anything(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        await cycles.add(a_full_cycle(COMPUTER_SCIENCE))
        await make_offer_to_applicant.execute(COMMAND)
...
class TestEverythingIsReadForTheApplicantsOwnSession:
    async def test_a_cycle_opened_for_another_session_is_not_this_applicants(
        self, make_offer_to_applicant: MakeOfferToApplicant, cycles: AdmissionCycleRepositoryPort
    ) -> None:
        await cycles.add(a_cycle(COMPUTER_SCIENCE, session_id=OTHER_SESSION_ID))

        with pytest.raises(AdmissionCycleNotFoundError):
            await make_offer_to_applicant.execute(COMMAND)

    async def test_a_chain_published_for_another_session_is_not_consulted(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        requirements: ProgramEntryRequirementRepositoryPort,
        ...
        await cycles.add(a_cycle(MATHEMATICS))
        await requirements.add(a_science_requirement(MATHEMATICS))
        await policies.add(a_policy((MATHEMATICS,), session_id=OTHER_SESSION_ID))
    ...
    async def test_an_alternatives_requirement_is_read_for_this_session(
        self,
        make_offer_to_applicant: MakeOfferToApplicant,
        cycles: AdmissionCycleRepositoryPort,
        requirements: ProgramEntryRequirementRepositoryPort,
        ...
        await cycles.add(a_full_cycle(COMPUTER_SCIENCE))
        await cycles.add(a_cycle(MATHEMATICS))
        await requirements.add(a_science_requirement(MATHEMATICS, session_id=OTHER_SESSION_ID))
        await policies.add(a_policy((MATHEMATICS,)))
