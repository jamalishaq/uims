...
class TestApplicantStorageContract:
    async def test_an_added_applicant_comes_back(self) -> None:
        repository = InMemoryApplicantRepository()
        applicant = an_applicant()

        await repository.add(applicant)

        assert await repository.get("app-0001") is applicant

    async def test_an_unknown_id_returns_none(self) -> None:
        """Absence is an answer, not a failure."""
        assert await InMemoryApplicantRepository().get("app-nobody") is None

    async def test_adding_a_second_applicant_under_the_same_id_is_refused(self) -> None:
        repository = InMemoryApplicantRepository()
        first = an_applicant()
        await repository.add(first)

        with pytest.raises(DuplicateAggregateError):
            await repository.add(an_applicant())

        assert await repository.get("app-0001") is first

    async def test_save_replaces_a_stored_applicant(self) -> None:
        repository = InMemoryApplicantRepository()
        await repository.add(an_applicant())
        ...
        assert await repository.get("app-0001") is replacement

    async def test_save_on_an_id_that_was_never_added_is_refused(self) -> None:
        repository = InMemoryApplicantRepository()

        with pytest.raises(AggregateNotFoundError):
            await repository.save(an_applicant())

        assert await repository.get("app-0001") is None

    async def test_a_screened_applicant_keeps_the_status_it_was_saved_with(self) -> None:
        repository = InMemoryApplicantRepository()
        applicant = an_applicant()
        await repository.add(applicant)
        applicant.screen()
        ...
        stored = await repository.get("app-0001")
        assert stored is not None
        assert stored.status is applicant.status


class TestListingApplicantsForASession:
    async def test_applications_are_filtered_by_session(self) -> None:
        repository = InMemoryApplicantRepository()
        this_year = an_applicant("app-0001")
        await repository.add(this_year)
        await repository.add(an_applicant("app-0002", OTHER_SESSION_ID))

        assert await repository.list_for_session(SESSION_ID) == (this_year,)

    async def test_applications_are_listed_in_insertion_order(self) -> None:
        repository = InMemoryApplicantRepository()
        later, earlier = an_applicant("app-0009"), an_applicant("app-0001")
        await repository.add(later)
        await repository.add(earlier)

        assert await repository.list_for_session(SESSION_ID) == (later, earlier)

    async def test_a_session_nobody_applied_in_lists_nothing(self) -> None:
        repository = InMemoryApplicantRepository()
        await repository.add(an_applicant())

        assert await repository.list_for_session("sess-1999") == ()


class TestEntryRequirementStorageContract:
    async def test_a_published_requirement_comes_back(self) -> None:
        repository = InMemoryProgramEntryRequirementRepository()
        requirement = a_requirement()

        await repository.add(requirement)

        assert await repository.get(PROGRAM_ID, SESSION_ID) is requirement

    async def test_an_unpublished_requirement_returns_none(self) -> None:
        assert await InMemoryProgramEntryRequirementRepository().get(PROGRAM_ID, SESSION_ID) is None

    async def test_publishing_twice_for_the_same_program_and_session_is_refused(self) -> None:
        repository = InMemoryProgramEntryRequirementRepository()
        first = a_requirement()
        await repository.add(first)

        with pytest.raises(DuplicateAggregateError):
            await repository.add(a_requirement())

        assert await repository.get(PROGRAM_ID, SESSION_ID) is first

    async def test_save_on_a_pair_that_was_never_published_is_refused(self) -> None:
        repository = InMemoryProgramEntryRequirementRepository()

        with pytest.raises(AggregateNotFoundError):
            await repository.save(a_requirement())

        assert await repository.get(PROGRAM_ID, SESSION_ID) is None


class TestRequirementsAreScopedToASession:
    async def test_one_program_can_hold_a_different_requirement_each_session(self) -> None:
        ...
        await repository.add(this_year)
        await repository.add(next_year)

        assert await repository.get(PROGRAM_ID, SESSION_ID) is this_year
        assert await repository.get(PROGRAM_ID, OTHER_SESSION_ID) is next_year

    async def test_two_programs_in_one_session_are_kept_apart(self) -> None:
        repository = InMemoryProgramEntryRequirementRepository()
        ...
        assert await repository.get("prg-csc", SESSION_ID) is computing
        assert await repository.get("prg-phy", SESSION_ID) is physics

    async def test_a_pair_is_not_confused_with_its_halves_joined_differently(self) -> None:
        """``("prg-a", "b-sess")`` and ``("prg-a-b", "sess")`` are different requirements."""
        repository = InMemoryProgramEntryRequirementRepository()
        await repository.add(a_requirement("prg-a", "b-sess"))

        assert await repository.get("prg-a-b", "sess") is None
        assert await repository.get("prg-a", "b-sess") is not None


class TestAdmissionCycleStorageContract:
    async def test_an_opened_cycle_comes_back(self) -> None:
        repository = InMemoryAdmissionCycleRepository()
        cycle = a_cycle()

        await repository.add(cycle)

        assert await repository.get(PROGRAM_ID, SESSION_ID) is cycle

    async def test_a_program_with_no_cycle_returns_none(self) -> None:
        """Different from a cycle with a quota of zero, which is a deliberate closed door."""
        assert await InMemoryAdmissionCycleRepository().get(PROGRAM_ID, SESSION_ID) is None

    async def test_opening_a_second_cycle_for_the_same_program_and_session_is_refused(self) -> None:
        repository = InMemoryAdmissionCycleRepository()
        first = a_cycle()
        await repository.add(first)

        with pytest.raises(DuplicateAggregateError):
            await repository.add(a_cycle())

        assert await repository.get(PROGRAM_ID, SESSION_ID) is first

    async def test_save_on_a_pair_that_was_never_opened_is_refused(self) -> None:
        repository = InMemoryAdmissionCycleRepository()

        with pytest.raises(AggregateNotFoundError):
            await repository.save(a_cycle())

        assert await repository.get(PROGRAM_ID, SESSION_ID) is None

    async def test_a_claimed_place_survives_being_saved(self) -> None:
        ...
        stored = await repository.get(PROGRAM_ID, SESSION_ID)
        assert stored is not None
        assert stored.offers_made == 1

    async def test_one_program_can_hold_a_different_cycle_each_session(self) -> None:
        repository = InMemoryAdmissionCycleRepository()
    ...
    async def test_two_programs_in_one_session_are_kept_apart(self) -> None:
        ...
        await repository.add(computing)
        await repository.add(mathematics)

        assert await repository.get(PROGRAM_ID, SESSION_ID) is computing
        assert await repository.get(OTHER_PROGRAM_ID, SESSION_ID) is mathematics


class TestAlternativeProgramPolicyStorageContract:
    async def test_a_published_chain_comes_back(self) -> None:
        repository = InMemoryAlternativeProgramPolicyRepository()
        policy = a_policy()

        await repository.add(policy)

        assert await repository.get(PROGRAM_ID, SESSION_ID) is policy

    async def test_a_program_with_no_published_chain_returns_none(self) -> None:
        assert (
            await InMemoryAlternativeProgramPolicyRepository().get(PROGRAM_ID, SESSION_ID) is None
        )
    ...
    async def test_save_on_a_pair_that_was_never_published_is_refused(self) -> None:
        ...
        with pytest.raises(AggregateNotFoundError):
            await repository.save(a_policy())

        assert await repository.get(PROGRAM_ID, SESSION_ID) is None

    async def test_one_program_can_overflow_somewhere_different_each_session(self) -> None:
    ...
    async def test_the_stored_chain_keeps_its_order(self) -> None:
        repository = InMemoryAlternativeProgramPolicyRepository()
...
class TestProgramInfoTranslationContract:
    ...
    async def test_a_registered_program_is_reported_as_admitting(self) -> None:
        adapter = InMemoryProgramInfoAdapter()
        adapter.register(PROGRAM_ID, SESSION_ID, admitting=True)

        assert await adapter.program_for(PROGRAM_ID, SESSION_ID) == ProgramInfo(PROGRAM_ID, True)

    async def test_a_closed_program_still_exists(self) -> None:
        ...
        info = await adapter.program_for(PROGRAM_ID, SESSION_ID)
        assert info is not None
        assert info.is_admitting is False

    async def test_an_unregistered_program_is_not_known(self) -> None:
        assert await InMemoryProgramInfoAdapter().program_for(PROGRAM_ID, SESSION_ID) is None

    async def test_a_program_admitting_one_session_need_not_admit_the_next(self) -> None:
        ...
        this_year = await adapter.program_for(PROGRAM_ID, SESSION_ID)
        next_year = await adapter.program_for(PROGRAM_ID, OTHER_SESSION_ID)
    ...
    async def test_registering_again_replaces_the_earlier_answer(self) -> None:
