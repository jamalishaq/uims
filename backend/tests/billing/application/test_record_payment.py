...
async def test_a_payment_lands_on_the_ledger_and_is_allocated(
    record_payment: RecordPayment, an_admitted_account: Account
) -> None:
    result = await record_payment.execute(a_command(amount=Decimal("5000")))
...
async def test_the_same_gateway_ref_recorded_twice_changes_the_ledger_once(
    record_payment: RecordPayment, an_admitted_account: Account
) -> None:
    """Phase 5.1's first claim, at the seam a retrying webhook actually hits."""
    first = await record_payment.execute(a_command())
    second = await record_payment.execute(a_command(received_at=SEPTEMBER))
...
async def test_an_overpayment_leaves_a_credit_balance(
    record_payment: RecordPayment, an_admitted_account: Account
) -> None:
    """Phase 5.1's second claim. Never assume balance ≤ charges."""
    await record_payment.execute(a_command(amount=Decimal("100000")))

    assert an_admitted_account.outstanding == Money.zero()
    assert an_admitted_account.credit_balance == Money("30000")


async def test_the_acceptance_fee_paid_event_fires_exactly_once_per_applicant(
    ...
    for index, amount in enumerate(["5000", "5000", "10000", "50000"]):
        await record_payment.execute(a_command(gateway_ref=f"ref-{index}", amount=Decimal(amount)))
    await record_payment.execute(a_command(gateway_ref="ref-2", amount=Decimal("10000")))

    assert events.published == (AcceptanceFeePaid(applicant_id=APPLICANT_ID),)


async def test_nothing_is_announced_before_the_acceptance_fee_is_settled(
    record_payment: RecordPayment,
    an_admitted_account: Account,
    events: InMemoryEventPublisher,
) -> None:
    await record_payment.execute(a_command(amount=Decimal("19999.99")))
...
async def test_a_duplicate_announces_nothing(
    ...
    await record_payment.execute(a_command())
    events.clear()

    result = await record_payment.execute(a_command())

    assert result.published == ()
    assert events.published == ()


async def test_the_event_names_the_applicant_admissions_is_waiting_on(
    record_payment: RecordPayment,
    an_admitted_account: Account,
    events: InMemoryEventPublisher,
) -> None:
...
async def test_a_payment_quoting_the_matric_number_reaches_the_same_ledger(
    record_payment: RecordPayment,
    an_admitted_account: Account,
    ...
    await accounts.save(an_admitted_account)

    result = await record_payment.execute(a_command(party_id=MATRIC_NUMBER))

    assert result.party_id == APPLICANT_ID
    assert an_admitted_account.total_paid == Money("20000")


async def test_money_for_a_party_with_no_ledger_is_refused(record_payment: RecordPayment) -> None:
    """Opening an account here would invent a party out of a gateway reference."""
    with pytest.raises(AccountNotFoundError):
        await record_payment.execute(a_command(party_id="app-9999"))


async def test_a_float_amount_is_refused(
    record_payment: RecordPayment, an_admitted_account: Account
) -> None:
    """An adapter that read a gateway's JSON number carelessly fails here, not three decimal
    places later."""
    with pytest.raises(InvalidAmountError):
        await record_payment.execute(a_command(amount=20000.5))


async def test_the_ledger_records_the_amount_that_arrived(
    record_payment: RecordPayment, an_admitted_account: Account
) -> None:
    ...
    await record_payment.execute(a_command(amount=Decimal("12500.50")))
