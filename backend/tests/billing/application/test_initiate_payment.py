...
async def test_a_checkout_is_recorded_as_initiated(
    initiate_payment: InitiatePayment,
    an_admitted_account: Account,
    intents: PaymentIntentRepositoryPort,
) -> None:
    result = await initiate_payment.execute(a_command())
    ...
    assert await intents.get(REFERENCE) is not None


async def test_opening_a_checkout_moves_no_money(
    ...
    await initiate_payment.execute(a_command())
...
async def test_the_ttl_defaults_to_billings_own(
    initiate_payment: InitiatePayment, an_admitted_account: Account
) -> None:
    assert (await initiate_payment.execute(a_command())).intent.ttl == DEFAULT_INTENT_TTL


async def test_a_ttl_may_be_given_at_the_call_site(
    initiate_payment: InitiatePayment, an_admitted_account: Account
) -> None:
    result = await initiate_payment.execute(a_command(ttl=timedelta(minutes=20)))

    assert result.intent.expires_at == JULY + timedelta(minutes=20)


async def test_money_asked_of_a_party_with_no_ledger_is_refused(
    initiate_payment: InitiatePayment, intents: PaymentIntentRepositoryPort
) -> None:
    with pytest.raises(AccountNotFoundError):
        await initiate_payment.execute(a_command(party_id="app-nobody"))

    assert await intents.get(REFERENCE) is None


async def test_a_reference_can_only_be_claimed_once(
    initiate_payment: InitiatePayment, an_admitted_account: Account
) -> None:
    ...
    with pytest.raises(DuplicateAggregateError):
        await initiate_payment.execute(a_command(amount=Decimal("999")))


async def test_an_intent_opened_against_a_matric_number_is_keyed_to_the_one_ledger(
    ...
    result = await initiate_payment.execute(a_command(party_id=MATRIC_NUMBER))
...
async def test_a_float_amount_is_refused(
    ...
    with pytest.raises(InvalidAmountError):
        await initiate_payment.execute(a_command(amount=20000.0))
