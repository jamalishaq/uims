...
async def test_an_accepted_offer_opens_a_ledger_with_both_admission_charges(
    open_account_for_offer: OpenAccountForOffer,
    accounts: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    result = await open_account_for_offer.execute(a_command())
...
async def test_only_the_acceptance_charge_gates_matriculation(
...
async def test_the_account_is_opened_for_the_offered_program_at_the_entry_level(
    open_account_for_offer: OpenAccountForOffer,
    accounts: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    """The offered program, which is not always the one applied for."""
    await open_account_for_offer.execute(a_command(program_id="prog-mcb-bsc"))
    stored = await accounts.get(APPLICANT_ID)
    assert stored is not None
...
async def test_a_level_may_be_stated_when_the_caller_knows_one(
    open_account_for_offer: OpenAccountForOffer,
    accounts: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    await open_account_for_offer.execute(a_command(level=200))
    stored = await accounts.get(APPLICANT_ID)
    assert stored is not None
    assert stored.level == Level(200)


async def test_the_amounts_come_from_the_schedule_and_not_from_the_caller(
    open_account_for_offer: OpenAccountForOffer,
    ...
    result = await open_account_for_offer.execute(a_command(session_id="sess-2027"))

    assert result.acceptance_charge.amount == Money("25000")
    assert result.matriculation_charge.amount == Money("55000")


async def test_a_redelivered_offer_opens_no_second_account_and_charges_nothing_twice(
    open_account_for_offer: OpenAccountForOffer,
    accounts: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    """An at-least-once bus replays; a second acceptance fee is the failure that would cause."""
    first = await open_account_for_offer.execute(a_command())
    second = await open_account_for_offer.execute(a_command())
...
async def test_an_unpriced_session_is_refused(
    ...
    with pytest.raises(FeeScheduleNotPublishedError):
        await open_account_for_offer.execute(a_command())
    assert await accounts.get(APPLICANT_ID) is None


async def test_opening_an_account_announces_nothing(
    open_account_for_offer: OpenAccountForOffer,
