...
async def a_cohort(accounts: AccountRepositoryPort) -> AccountRepositoryPort:
    ...
    await accounts.add(Account.open("app-0002", MCB_PROGRAM_ID))
    await accounts.add(Account.open("app-0003", LAW_PROGRAM_ID))
    return accounts


async def test_charges_every_account_the_schedule_prices(
    apply_session_fees: ApplySessionFees,
    a_cohort: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    result = await apply_session_fees.execute(SESSION_2026)

    assert result.charged == ("app-0001", "app-0002")
    csc = await a_cohort.get("app-0001")
    assert csc is not None
    assert csc.charge_for(ChargeKind.SESSION, SESSION_2026) is not None


async def test_charges_each_program_and_level_its_own_amount(
    apply_session_fees: ApplySessionFees,
    a_cohort: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    await apply_session_fees.execute(SESSION_2026)

    csc, mcb = await a_cohort.get("app-0001"), await a_cohort.get("app-0002")
...
async def test_an_account_the_schedule_does_not_price_is_skipped_and_named(
    apply_session_fees: ApplySessionFees,
    a_cohort: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    ...
    law = await a_cohort.get("app-0003")
    assert law is not None
    assert law.charges == ()


async def test_an_account_at_an_unpriced_level_is_skipped_too(
    apply_session_fees: ApplySessionFees,
    accounts: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
    """The key is the pair: a priced program at an unpriced level is still unpriced."""
    await accounts.add(Account.open("app-0009", CSC_PROGRAM_ID, level=Level(500)))
    result = await apply_session_fees.execute(SESSION_2026)
...
async def test_a_redelivered_session_charges_nobody_twice(
    ...
    first = await apply_session_fees.execute(SESSION_2026)
    second = await apply_session_fees.execute(SESSION_2026)
...
async def test_a_rerun_after_a_partial_batch_finishes_the_job(
    apply_session_fees: ApplySessionFees,
    accounts: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
...
async def test_a_session_with_no_schedule_at_all_stops_the_batch(
...
async def test_the_batch_reports_everything_it_considered(
    apply_session_fees: ApplySessionFees,
    a_cohort: AccountRepositoryPort,
    published_schedule: FeeSchedule,
) -> None:
...
async def test_next_session_is_another_charge_on_the_same_ledger(
    apply_session_fees: ApplySessionFees,
    accounts: AccountRepositoryPort,
    ...
    await apply_session_fees.execute(SESSION_2026)
    await apply_session_fees.execute("sess-2027")

    stored = await accounts.get("app-0001")
    assert stored is not None
    assert len(stored.charges) == 2
    assert stored.outstanding == Money("210000")


async def test_the_batch_announces_nothing(
    apply_session_fees: ApplySessionFees,
    a_cohort: AccountRepositoryPort,
