...
class TestBankingTheMoney:
    def test_a_confirmation_puts_the_money_on_the_ledger_and_moves_the_intent(
        self,
        confirm_payment: ConfirmPayment,
        an_open_intent: PaymentIntent,
        an_admitted_account: Account,
    ) -> None:
        result = confirm_payment.execute(a_command())
    ...
    def test_the_ledger_records_the_confirmed_amount_and_not_the_intents(
        self,
        confirm_payment: ConfirmPayment,
        an_open_intent: PaymentIntent,
        an_admitted_account: Account,
    ) -> None:
        confirm_payment.execute(a_command(amount=Decimal("12000")))
    ...
    def test_settling_the_gating_charge_is_announced(
        self,
        confirm_payment: ConfirmPayment,
        an_open_intent: PaymentIntent,
        ...
        confirm_payment.execute(a_command())

        assert events.published == (AcceptanceFeePaid(applicant_id=APPLICANT_ID),)

    def test_a_replay_reaches_the_ledgers_existing_no_op(
        self,
        confirm_payment: ConfirmPayment,
        an_open_intent: PaymentIntent,
        an_admitted_account: Account,
        ...
        replay = confirm_payment.execute(a_command())
    ...
    def test_the_party_is_taken_from_the_intent(
        self,
        confirm_payment: ConfirmPayment,
...
class TestOrdering:
    ...
    class SaveRefusingIntents(PaymentIntentRepositoryPort):
        ...
        def all_initiated(self) -> tuple[PaymentIntent, ...]:
            return self._inner.all_initiated()

    def test_the_money_is_on_the_ledger_even_when_the_intent_cannot_be_saved(
        self,
        intents: PaymentIntentRepositoryPort,
...
class TestRefusals:
    ...
    def test_a_reported_failure_leaves_the_ledger_alone(
        self,
        confirm_payment: ConfirmPayment,
        an_open_intent: PaymentIntent,
        an_admitted_account: Account,
        ...
        result = confirm_payment.execute(
    ...
    def test_a_gateway_that_confirms_then_fails_is_refused(
        self, confirm_payment: ConfirmPayment, an_open_intent: PaymentIntent
    ) -> None:
        confirm_payment.execute(a_command())

        with pytest.raises(PaymentIntentFinalError):
            confirm_payment.execute(a_command(succeeded=False, amount=None))

    def test_a_gateway_that_fails_then_confirms_banks_the_money_and_then_raises(
        self,
        confirm_payment: ConfirmPayment,
        an_open_intent: PaymentIntent,
        an_admitted_account: Account,
