...
def a_graph(*courses: Course) -> PrerequisiteGraph:
    ...
    return PrerequisiteGraph(catalog.get)


class TestReadingAChain:
    def test_a_course_with_no_prerequisites_has_an_empty_chain(self) -> None:
        intro = a_course("crs-csc-101", "CSC101")

        assert a_graph(intro).chain_for("crs-csc-101") == ()

    def test_a_direct_prerequisite_is_reported(self) -> None:
        intro = a_course("crs-csc-101", "CSC101")
        data_structures = a_course("crs-csc-201", "CSC201", "crs-csc-101")

        chain = a_graph(intro, data_structures).chain_for("crs-csc-201")

        assert chain == ("crs-csc-101",)

    def test_a_chain_is_followed_all_the_way_down(self) -> None:
        """The verification criterion: prerequisite chains can be read."""
        intro = a_course("crs-csc-101", "CSC101")
        data_structures = a_course("crs-csc-201", "CSC201", "crs-csc-101")
        algorithms = a_course("crs-csc-301", "CSC301", "crs-csc-201")

        chain = a_graph(intro, data_structures, algorithms).chain_for("crs-csc-301")

        assert chain == ("crs-csc-201", "crs-csc-101")

    def test_nearer_prerequisites_come_first(self) -> None:
        intro = a_course("crs-csc-101", "CSC101")
        maths = a_course("crs-mth-101", "MTH101")
        data_structures = a_course("crs-csc-201", "CSC201", "crs-csc-101")
        algorithms = a_course("crs-csc-301", "CSC301", "crs-csc-201", "crs-mth-101")

        chain = a_graph(intro, maths, data_structures, algorithms).chain_for("crs-csc-301")

        assert chain == ("crs-csc-201", "crs-mth-101", "crs-csc-101")

    def test_a_diamond_reports_each_course_once(self) -> None:
        ...
        chain = a_graph(intro, data_structures, systems, project).chain_for("crs-csc-401")

        assert chain == ("crs-csc-201", "crs-csc-202", "crs-csc-101")

    def test_the_course_itself_is_never_in_its_own_chain(self) -> None:
        intro = a_course("crs-csc-101", "CSC101")
        data_structures = a_course("crs-csc-201", "CSC201", "crs-csc-101")

        assert "crs-csc-201" not in a_graph(intro, data_structures).chain_for("crs-csc-201")

    def test_an_unresolvable_prerequisite_id_contributes_nothing(self) -> None:
        """Whether a stored id ought to resolve is the application layer's question."""
        data_structures = a_course("crs-csc-201", "CSC201", "crs-csc-101")

        assert a_graph(data_structures).chain_for("crs-csc-201") == ("crs-csc-101",)

    def test_an_unknown_course_has_an_empty_chain(self) -> None:
        assert a_graph().chain_for("crs-nobody") == ()
...
class TestChainsOverCyclicData:
    ...
    def test_a_two_course_loop_terminates(self) -> None:
        first = a_course("crs-csc-101", "CSC101", "crs-csc-201")
        second = a_course("crs-csc-201", "CSC201", "crs-csc-101")

        assert a_graph(first, second).chain_for("crs-csc-101") == ("crs-csc-201",)

    def test_a_longer_loop_terminates(self) -> None:
        first = a_course("crs-csc-101", "CSC101", "crs-csc-301")
        second = a_course("crs-csc-201", "CSC201", "crs-csc-101")
        third = a_course("crs-csc-301", "CSC301", "crs-csc-201")

        chain = a_graph(first, second, third).chain_for("crs-csc-101")

        assert chain == ("crs-csc-301", "crs-csc-201")


class TestRefusingACycle:
    def test_an_unrelated_prerequisite_is_allowed(self) -> None:
        intro = a_course("crs-csc-101", "CSC101")
        data_structures = a_course("crs-csc-201", "CSC201")

        a_graph(intro, data_structures).ensure_can_require(data_structures, "crs-csc-101")

    def test_requiring_a_course_that_already_requires_you_is_refused(self) -> None:
        intro = a_course("crs-csc-101", "CSC101")
        data_structures = a_course("crs-csc-201", "CSC201", "crs-csc-101")

        with pytest.raises(PrerequisiteCycleError):
            a_graph(intro, data_structures).ensure_can_require(intro, "crs-csc-201")

    def test_a_cycle_further_down_the_chain_is_refused(self) -> None:
        ...
        with pytest.raises(PrerequisiteCycleError):
            a_graph(intro, data_structures, algorithms).ensure_can_require(intro, "crs-csc-301")

    def test_direct_self_reference_is_left_to_the_entity(self) -> None:
        """The graph stays silent so ``Course`` can say it more precisely."""
        intro = a_course("crs-csc-101", "CSC101")

        a_graph(intro).ensure_can_require(intro, "crs-csc-101")
