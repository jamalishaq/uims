...
def graded(
    ...
    record_submitted_grade.execute(
        RecordSubmittedGradeCommand(
...
def test_a_correction_changes_the_stored_grade(
    correct_grade: CorrectGrade, records: AcademicRecordRepositoryPort
) -> None:
    correct_grade.execute(a_command())
...
def test_a_correction_reports_the_cgpa_and_standing_it_produced(
    correct_grade: CorrectGrade,
) -> None:
    """The reason anyone corrects a grade, answered without a second round trip."""
    result = correct_grade.execute(a_command())
...
def test_a_correction_can_move_a_student_off_probation(
    correct_grade: CorrectGrade, records: AcademicRecordRepositoryPort
) -> None:
    before = records.get(STUDENT_ID)
    assert before is not None
    assert before.standing is Standing.PROBATION

    correct_grade.execute(a_command())

    assert records.get(STUDENT_ID).standing is Standing.GOOD_STANDING  # type: ignore[union-attr]


def test_the_correction_is_kept_on_the_stored_record(
    correct_grade: CorrectGrade, records: AcademicRecordRepositoryPort
) -> None:
...
def test_correcting_a_student_with_no_record_is_refused(correct_grade: CorrectGrade) -> None:
    with pytest.raises(AcademicRecordNotFoundError, match="stu-nobody"):
        correct_grade.execute(a_command(student_id="stu-nobody"))


def test_correcting_a_course_the_record_does_not_hold_is_refused(
    correct_grade: CorrectGrade,
) -> None:
    with pytest.raises(GradeNotRecordedError):
        correct_grade.execute(a_command(course_id="PHY999"))
...
def test_a_correction_without_a_reason_or_an_authorizer_is_refused(
    ...
    with pytest.raises(InvalidCorrectionError):
        correct_grade.execute(a_command(reason=reason, authorized_by=authorized_by))
    assert records.get(STUDENT_ID).grades[0].score == 30  # type: ignore[union-attr]


def test_a_correction_that_changes_nothing_is_refused(correct_grade: CorrectGrade) -> None:
    with pytest.raises(InvalidCorrectionError, match="already recorded"):
        correct_grade.execute(a_command(corrected_score=30))


def test_the_use_case_loads_the_stored_record_rather_than_trusting_its_caller(
    correct_grade: CorrectGrade, records: AcademicRecordRepositoryPort
) -> None:
    """The command carries identifiers only — there is no way to hand it a record."""
    assert not hasattr(a_command(), "record")
    correct_grade.execute(a_command())
    assert records.get(STUDENT_ID).grades[0].score == 62  # type: ignore[union-attr]
