...
def test_the_view_carries_the_averages_and_the_standing(
    read_academic_record: ReadAcademicRecord,
) -> None:
    view = read_academic_record.execute(STUDENT_ID)
...
def test_the_view_answers_in_passes_rather_than_letters(
    read_academic_record: ReadAcademicRecord,
) -> None:
...
def test_a_carried_over_course_is_passed_while_both_attempts_still_count(
    read_academic_record: ReadAcademicRecord,
) -> None:
    view = read_academic_record.execute(STUDENT_ID)
...
def test_the_view_is_a_dto_rather_than_the_aggregate(
    read_academic_record: ReadAcademicRecord,
) -> None:
...
def test_reading_a_student_nobody_has_graded_is_an_error(
    read_academic_record: ReadAcademicRecord,
) -> None:
    with pytest.raises(AcademicRecordNotFoundError, match="stu-nobody"):
        read_academic_record.execute("stu-nobody")


def test_find_answers_none_for_a_student_nobody_has_graded(
    read_academic_record: ReadAcademicRecord,
) -> None:
    ...
    """
    assert read_academic_record.find("stu-nobody") is None
    assert read_academic_record.find(STUDENT_ID) is not None
