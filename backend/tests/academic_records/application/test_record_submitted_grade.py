...
async def test_a_students_first_grade_opens_their_record(
    ...
    result = await record_submitted_grade.execute(a_command())
...
async def test_the_line_is_weighed_with_the_units_the_catalog_answered(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    """Credit units are looked up, never taken from the caller: the command has no such field."""
    await record_submitted_grade.execute(a_command())
    await record_submitted_grade.execute(a_command(course_id="MTH101", score=62))
...
async def test_a_second_grade_is_added_to_the_record_that_already_exists(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    await record_submitted_grade.execute(a_command())
    result = await record_submitted_grade.execute(a_command(course_id="MTH101", score=62))
...
async def test_a_grade_for_a_course_the_catalog_does_not_know_is_refused(
    ...
    with pytest.raises(CourseCreditsUnavailableError, match="PHY999"):
        await record_submitted_grade.execute(a_command(course_id="PHY999"))
    assert await records.get(STUDENT_ID) is None


async def test_the_catalog_is_consulted_before_a_record_is_opened(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    ...
    with pytest.raises(CourseCreditsUnavailableError):
        await record_submitted_grade.execute(a_command(course_id="PHY999"))
    with pytest.raises(CourseCreditsUnavailableError):
        await record_submitted_grade.execute(a_command(course_id="PHY999", student_id="stu-other"))
    assert await records.get("stu-other") is None


# ---- redelivery ----


async def test_replaying_the_same_grade_leaves_exactly_one_line(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    """A bus that guarantees at-least-once delivery will replay this. Twice is not an error."""
    await record_submitted_grade.execute(a_command())
    replay = await record_submitted_grade.execute(a_command())
...
async def test_a_replay_does_not_move_the_cgpa(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    await record_submitted_grade.execute(a_command())
    before = (await records.get(STUDENT_ID)).cgpa  # type: ignore[union-attr]
    await record_submitted_grade.execute(a_command())
    assert (await records.get(STUDENT_ID)).cgpa == before  # type: ignore[union-attr]


async def test_a_different_score_for_a_course_already_graded_is_refused(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    ...
    with pytest.raises(GradeAlreadyRecordedError):
        await record_submitted_grade.execute(a_command(score=30))
...
async def test_the_same_course_in_a_later_semester_is_a_carry_over_and_both_count(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    """Confirmed rule: every attempt is a line and every line counts."""
    await record_submitted_grade.execute(a_command(score=30))
    await record_submitted_grade.execute(a_command(semester_id="sem-2027-1", score=55))
...
async def test_a_failing_students_record_comes_out_on_probation(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    await record_submitted_grade.execute(a_command(score=42))
    await record_submitted_grade.execute(a_command(course_id="MTH101", score=30))
...
async def test_two_students_get_two_records(
    record_submitted_grade: RecordSubmittedGrade, records: AcademicRecordRepositoryPort
) -> None:
    await record_submitted_grade.execute(a_command())
    await record_submitted_grade.execute(a_command(student_id="stu-2026-0002", score=30))

    assert (await records.get(STUDENT_ID)).cgpa == Decimal("5.00")  # type: ignore[union-attr]
    assert (await records.get("stu-2026-0002")).cgpa == Decimal("0.00")  # type: ignore[union-attr]
