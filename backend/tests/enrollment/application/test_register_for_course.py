...
@pytest.fixture(autouse=True)
async def open_offerings(offerings: CourseOfferingRepositoryPort) -> CourseOfferingRepositoryPort:
    """Every course is being run this term, with room in it."""
    for course_id in (CSC101, CSC201, MTH101, PHY101):
        await offerings.add(CourseOffering.open(course_id, FIRST, capacity=40))
    return offerings
...
async def fill_the_term(
    ...
    for index, course_id in enumerate(course_ids):
        outcome = await register_for_course.execute(
...
class TestAPermittedRegistration:
    async def test_the_student_is_registered_and_a_seat_is_claimed(
        self,
        register_for_course: RegisterForCourse,
        enrollments: EnrollmentRepositoryPort,
        offerings: CourseOfferingRepositoryPort,
    ) -> None:
        outcome = await register_for_course.execute(a_command())

        assert isinstance(outcome, RegistrationAccepted)
        assert (outcome.student_id, outcome.course_id, outcome.term) == (STUDENT_ID, CSC201, FIRST)
        assert outcome.credit_units == 3

        stored = await enrollments.get("enr-0001")
        assert isinstance(stored, Enrollment)
        assert stored.status is EnrollmentStatus.REGISTERED

        offering = await offerings.get(CSC201, FIRST)
        assert offering is not None
        assert offering.seats_taken == 1
        assert outcome.seats_remaining == 39

    async def test_exactly_one_seat_goes_per_registration(
        self, register_for_course: RegisterForCourse, offerings: CourseOfferingRepositoryPort
    ) -> None:
        for index in range(3):
            await register_for_course.execute(
                a_command(MTH101, enrollment_id=f"enr-{index}", student_id=f"stu-{index}")
            )
        offering = await offerings.get(MTH101, FIRST)
        assert offering is not None
        assert offering.seats_taken == 3

    async def test_a_fresher_with_no_academic_record_can_register(
        self, register_for_course: RegisterForCourse
    ) -> None:
        """No record means passed nothing, not refused: this is everyone's first semester."""
        outcome = await register_for_course.execute(a_command(MTH101, student_id="stu-fresher"))
        assert isinstance(outcome, RegistrationAccepted)

    async def test_registering_the_same_course_in_a_later_term_is_a_different_registration(
        self,
        register_for_course: RegisterForCourse,
        offerings: CourseOfferingRepositoryPort,
    ) -> None:
        await offerings.add(CourseOffering.open(MTH101, SECOND, capacity=40))
        await register_for_course.execute(a_command(MTH101, enrollment_id="enr-first"))
        outcome = await register_for_course.execute(
...
class TestRefusedForAnUnmetPrerequisite:
    async def test_a_student_who_has_not_passed_the_prerequisite_is_refused(
        self, register_for_course: RegisterForCourse
    ) -> None:
        outcome = await register_for_course.execute(a_command(CSC201, student_id="stu-no-history"))
    ...
    async def test_the_refusal_writes_nothing(
        self,
        register_for_course: RegisterForCourse,
        enrollments: EnrollmentRepositoryPort,
        offerings: CourseOfferingRepositoryPort,
        ...
        await register_for_course.execute(a_command(CSC201, student_id="stu-no-history"))
        offering = await offerings.get(CSC201, FIRST)
        assert offering is not None
        assert offering.seats_taken == 0
        assert await enrollments.get("enr-0001") is None


class TestRefusedForGoingOverTheCreditCap:
    async def test_the_registration_that_lands_exactly_on_the_cap_is_accepted(
        self,
        register_for_course: RegisterForCourse,
        ...
        await offerings.add(CourseOffering.open("crs-load21", FIRST, capacity=40))
        await register_for_course.execute(a_command("crs-load21", enrollment_id="enr-load"))

        assert isinstance(
            await register_for_course.execute(a_command(CSC201)), RegistrationAccepted
        )

    async def test_the_registration_that_passes_the_cap_is_refused(
        self,
        register_for_course: RegisterForCourse,
        courses: InMemoryCourseInfoAdapter,
        offerings: CourseOfferingRepositoryPort,
        ...
        await offerings.add(CourseOffering.open("crs-big", FIRST, capacity=40))
        await register_for_course.execute(a_command("crs-big", enrollment_id="enr-big"))

        outcome = await register_for_course.execute(a_command(CSC201))
        assert isinstance(outcome, RegistrationRefused)
    ...
    async def test_the_cap_is_measured_over_the_term_and_not_the_session(
        self,
        register_for_course: RegisterForCourse,
        courses: InMemoryCourseInfoAdapter,
        offerings: CourseOfferingRepositoryPort,
        ...
        courses.register("crs-big", credit_units=24)
        await offerings.add(CourseOffering.open("crs-big", FIRST, capacity=40))
        await offerings.add(CourseOffering.open(CSC201, SECOND, capacity=40))
        await register_for_course.execute(a_command("crs-big", enrollment_id="enr-big"))

        outcome = await register_for_course.execute(a_command(CSC201, term=SECOND))
        assert isinstance(outcome, RegistrationAccepted)


class TestRefusedForWantOfFinancialClearance:
    async def test_a_student_billing_has_not_cleared_is_refused(
        self,
        register_for_course: RegisterForCourse,
    ...
    async def test_clearance_is_asked_per_term_not_per_session(
        self,
        register_for_course: RegisterForCourse,
        clearance: StubFinancialClearanceAdapter,
        offerings: CourseOfferingRepositoryPort,
        ...
        assert isinstance(
            await register_for_course.execute(
                a_command(CSC201, enrollment_id="enr-2", term=SECOND)
            ),
            RegistrationRefused,
        )

    async def test_the_stub_clears_everyone_it_has_not_been_told_about(
        self, register_for_course: RegisterForCourse
    ) -> None:
        """The Phase 5 stand-in: cleared until Billing exists to say otherwise."""
        assert isinstance(
            await register_for_course.execute(a_command(MTH101, student_id="stu-unknown")),
            RegistrationAccepted,
        )


class TestRefusedForCapacity:
    async def test_the_student_who_takes_the_last_seat_gets_it_and_the_next_one_does_not(
        self,
        register_for_course: RegisterForCourse,
        courses: InMemoryCourseInfoAdapter,
        offerings: CourseOfferingRepositoryPort,
        ...
        await offerings.add(CourseOffering.open("crs-popular", FIRST, capacity=1))

        first = await register_for_course.execute(
            a_command("crs-popular", enrollment_id="enr-lucky", student_id="stu-a")
        )
        second = await register_for_course.execute(
        ...
        assert await enrollments.get("enr-late") is None

    async def test_a_full_course_stays_exactly_as_full_as_it_was(
        self, register_for_course: RegisterForCourse, offerings: CourseOfferingRepositoryPort
    ) -> None:
...
class TestRepeatsAndCarryOvers:
    async def test_the_same_course_cannot_be_registered_twice_in_one_term(
        self, register_for_course: RegisterForCourse, offerings: CourseOfferingRepositoryPort
    ) -> None:
        await register_for_course.execute(a_command(CSC201))
        outcome = await register_for_course.execute(a_command(CSC201, enrollment_id="enr-again"))
    ...
    async def test_a_course_already_passed_cannot_be_taken_again(
        self, register_for_course: RegisterForCourse
    ) -> None:
        outcome = await register_for_course.execute(a_command(CSC101, enrollment_id="enr-repeat"))
        assert isinstance(outcome, RegistrationRefused)
        assert outcome.has(EligibilityReason.ALREADY_PASSED)

    async def test_a_course_previously_failed_comes_back_as_a_carry_over(
        self,
        register_for_course: RegisterForCourse,
        offerings: CourseOfferingRepositoryPort,
    ) -> None:
        ...
        await offerings.add(CourseOffering.open(CSC201, SECOND, capacity=40))
        first_attempt = await register_for_course.execute(a_command(CSC201))
        assert isinstance(first_attempt, RegistrationAccepted)
        assert not first_attempt.is_carry_over

        retake = await register_for_course.execute(
...
class TestQuestionsThatCannotBeAnswered:
    async def test_a_course_the_catalog_does_not_know_is_an_error(
        self, register_for_course: RegisterForCourse
    ) -> None:
        with pytest.raises(CourseNotFoundError):
            await register_for_course.execute(a_command("crs-nonexistent"))

    async def test_a_course_nobody_is_running_this_term_is_an_error(
        self, register_for_course: RegisterForCourse
    ) -> None:
        """Different from a full course, which is a decision with a number behind it."""
        with pytest.raises(CourseOfferingNotFoundError):
            await register_for_course.execute(a_command(CSC201, term=SECOND))

    async def test_a_retired_course_is_refused_rather_than_raised(
        self,
        register_for_course: RegisterForCourse,
        courses: InMemoryCourseInfoAdapter,
    ) -> None:
        ...
        outcome = await register_for_course.execute(a_command(MTH101))
        assert isinstance(outcome, RegistrationRefused)
        assert outcome.has(EligibilityReason.COURSE_NOT_ACTIVE)


class TestEveryReasonAtOnce:
    async def test_a_student_who_is_wrong_in_several_ways_is_told_all_of_them(
        self,
        register_for_course: RegisterForCourse,
        clearance: StubFinancialClearanceAdapter,
    ) -> None:
...
class TestNoScheduleAwareness:
    async def test_two_courses_are_registrable_without_anyone_asking_when_they_meet(
        self, register_for_course: RegisterForCourse
    ) -> None:
        """Model A is permanent: registration is by eligibility, the timetable comes after."""
        assert isinstance(
            await register_for_course.execute(a_command(MTH101, enrollment_id="enr-a")),
            RegistrationAccepted,
        )
        assert isinstance(
            await register_for_course.execute(a_command(PHY101, enrollment_id="enr-b")),
            RegistrationAccepted,
        )
